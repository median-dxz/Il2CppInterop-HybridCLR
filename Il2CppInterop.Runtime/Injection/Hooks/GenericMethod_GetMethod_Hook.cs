            ...
            if (genericMethodGetMethod == 0) {
                ...
                if (getVirtualMethodXrefs.Length == 0) {
                    ...
                    if (UnityVersionHandler.HasShimForGetMethod) {
                        ...
                        var hybridCLRSignature = new MemoryUtils.SignatureDefinition
                        {
                            pattern = "\x48\x79\x62\x72\x69\x64\x43\x4C\x52\x2E\x52\x75\x6E\x74\x69\x6D\x65\x41\x70\x69",
                            mask = "xxxxxxxxxxxxxxxxxxxx",
                            xref = false,
                        };

                        // That's indicate that the game use the HybridCLR
                        if (MemoryUtils.FindSignatureInModule(InjectorHelpers.Il2CppModule, hybridCLRSignature) != 0) {
                            shimXrefs = XrefScannerLowLevel.JumpTargets(shimXrefs.Last(), true).ToArray();
                            genericMethodGetMethod = shimXrefs.First();
                        } else {
